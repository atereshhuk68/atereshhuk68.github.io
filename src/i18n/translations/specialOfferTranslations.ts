export const specialOfferTranslations = {
  pl: {
    "specialOffer.heading": "Cześć, Piękna!",
    "specialOffer.p1": "Czas zadbać o siebie",
    "specialOffer.p2": "Tylko teraz 15% zniżki na pierwszą wizytę!",
    "specialOffer.cta": "Umów się już dziś, żeby nie przegapić tej okazji!",
    "specialOffer.submitText": "Umów się teraz",
    "specialOffer.later": "Nie, dziękuję. Może później",
    "specialOffer.serviceSelect": "Wybierz kategorię usługi",
  },
  en: {
    "specialOffer.heading": "Hey, Beautiful!",
    "specialOffer.p1": "Time to take care of yourself",
    "specialOffer.p2": "15% off your first visit!",
    "specialOffer.cta": "Book today to not miss out on this offer!",
    "specialOffer.submitText": "Book now",
    "specialOffer.later": "No, thanks. Maybe later",
    "specialOffer.serviceSelect": "Select a service category",
  },
  uk: {
    "specialOffer.heading": "Привіт, красуне!",
    "specialOffer.p1": "Час подбати про себе",
    "specialOffer.p2": "15% знижки на перший візит!",
    "specialOffer.cta": "Запишись сьогодні, щоб не пропустити цю пропозицію!",
    "specialOffer.submitText": "Записатися зараз",
    "specialOffer.later": "Ні, дякую. Можливо пізніше",
    "specialOffer.serviceSelect": "Виберіть категорію послуги",
  },
  ru: {
    "specialOffer.heading": "Привет, красотка!",
    "specialOffer.p1": "Пора позаботиться о себе",
    "specialOffer.p2": "15% скидка на первый визит!",
    "specialOffer.cta":
      "Запишись сегодня, чтобы не пропустить это предложение!",
    "specialOffer.submitText": "Записаться сейчас",
    "specialOffer.later": "Нет, спасибо. Возможно позже",
    "specialOffer.serviceSelect": "Выберите категорию услуги",
  },
} as const;
